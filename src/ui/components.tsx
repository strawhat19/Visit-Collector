...
export const IconButton = ({ id, className, icon: Icon, label, onPress, palette, active, disabled = false, small = false }: ElementIdentity & {
  ...
  return (
    <Pressable
      {...elementProps(className ?? `icon-button`, identity)}
      disabled={disabled}
      onPress={onPress}
    ...
    >
      <Icon
        {...elementProps(`icon-button-symbol`, identity)}
        size={small ? 18 : 21}
        strokeWidth={1.7}
...
export const Card = ({ id, className, children, palette, style, testID }: ElementIdentity & { testID?: string; palette: Palette; children: ReactNode; style?: StyleProp<ViewStyle> }) => {
  ...
  return (
    <View
      {...elementProps(className ?? `dashboard-card`, id ?? scope)}
      testID={testID ?? `card`}
      style={[ui.card, { borderColor: palette.border, backgroundColor: palette.card }, style]}
    >
      {children}
    </View>
  );
...
export const Segmented = <T extends string,>({ id, className, options, value, onChange, palette, small = false, stretch = false }: ElementIdentity & {
  ...
  return (
    <View
      {...elementProps(className ?? `segmented-tabs`, identity)}
      accessibilityRole={`tablist`}
      style={[ui.segmented, stretch && { width: `100%`, alignSelf: `stretch` }, { borderColor: palette.border, backgroundColor: palette.bg }]}
    >
      {options.map(({ value: optionValue, label, icon: Icon }) => (
        <Pressable
          key={optionValue}
          {...elementProps(`segmented-tab`, `${identity}-${optionValue}`)}
        ...
        >
          {Icon && (
            <Icon
              {...elementProps(`segmented-tab-icon`, `${identity}-${optionValue}`)}
              size={15}
              strokeWidth={1.8}
              color={value === optionValue ? palette.blue : palette.muted}
            />
          )}
          <Text
            {...elementProps(`segmented-tab-label`, `${identity}-${optionValue}`)}
            style={[ui.segmentText, { color: value === optionValue ? palette.blue : palette.muted }]}
          >
            {label}
          </Text>
        </Pressable>
...
export const StatusBadge = ({ id, className, palette, demo = false }: ElementIdentity & { demo?: boolean; palette: Palette }) => {
  ...
  return (
    <View
      {...elementProps(className ?? `status-badge`, identity)}
      style={[ui.badge, { backgroundColor: demo ? palette.selected : `${palette.green}14` }]}
    >
      <View
        {...elementProps(`status-badge-dot`, identity)}
        style={[ui.dot, { backgroundColor: demo ? palette.blue : palette.green }]}
      />
      <Text
        {...elementProps(`status-badge-label`, identity)}
        style={[ui.badgeText, { color: demo ? palette.blue : palette.green }]}
      >
        {demo ? `Demo` : `Live`}
      </Text>
    </View>
...
export const TextButton = ({ id, className, children, onPress, palette, icon: Icon = ChevronRight }: ElementIdentity & { palette: Palette; children: ReactNode; icon?: LucideIcon; onPress: () => void }) => {
  ...
  return (
    <Pressable
      {...elementProps(className ?? `text-button`, identity)}
      onPress={onPress}
      style={ui.textButton}
      accessibilityRole={`button`}
    >
      <Text
        {...elementProps(`text-button-label`, identity)}
        style={{ fontSize: 13, fontWeight: `500`, color: palette.blue }}
      >
        {children}
      </Text>
      <Icon
        {...elementProps(`text-button-icon`, identity)}
        size={15}
        color={palette.blue}
        strokeWidth={1.8}
      />
