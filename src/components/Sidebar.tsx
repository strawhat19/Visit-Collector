...
const Sidebar = ({ items, active, expanded, palette, reducedMotion, onHome, onToggle, onSettings, onNavigate }: SidebarProps) => {
  const scope = useId();
  ...
  const button = (title: string, Icon: LucideIcon, onPress: () => void, id: string, selected = false, toggle = false) => (
    <Pressable
      {...elementProps(`sidebar-button`, `${scope}-${id}`)}
      key={id}
      onPress={onPress}
      accessibilityRole={`button`}
      aria-pressed={toggle ? undefined : selected}
      aria-expanded={toggle ? expanded : undefined}
      accessibilityState={{ selected, expanded: toggle ? expanded : undefined }}
      accessibilityLabel={toggle ? `${expanded ? `Collapse` : `Expand`} Sidebar` : title}
      style={({ pressed }) => [styles.button, { opacity: pressed ? 0.65 : 1, backgroundColor: selected ? palette.selected : `transparent` }]}
    >
      <View
        {...elementProps(`sidebar-button-icon-wrap`, `${scope}-${id}`)}
        style={styles.icon}
      >
        <Icon
          {...elementProps(`sidebar-button-icon`, `${scope}-${id}`)}
          size={22}
          strokeWidth={1.7}
      ...
      <Animated.Text
        {...elementProps(`sidebar-button-label`, `${scope}-${id}`)}
        accessible={false}
        numberOfLines={1}
        style={[styles.label, labelStyle, { color: selected ? palette.blue : palette.muted }]}
      >
        {title}
      </Animated.Text>
    </Pressable>
  ...
  return (
    <Animated.View
      {...elementProps(`sidebar`, scope)}
      testID={`sidebar`}
      style={[styles.sidebar, { borderColor: palette.border, backgroundColor: palette.rail, width: progress.interpolate({ inputRange: [0, 1], outputRange: [68, 236] }) }]}
    >
      <Pressable
        {...elementProps(`sidebar-brand`, scope)}
        onPress={onHome}
        style={styles.brand}
        accessibilityRole={`button`}
      ...
      >
        <View
          {...elementProps(`sidebar-logo-wrap`, scope)}
          style={styles.icon}
        >
          <Logo
            size={42}
        ...
        <Animated.Text
          {...elementProps(`sidebar-brand-label`, scope)}
          accessible={false}
          numberOfLines={1}
          accessibilityElementsHidden={!expanded}
          importantForAccessibility={expanded ? `auto` : `no-hide-descendants`}
          aria-hidden={!expanded}
          style={[styles.brandLabel, labelStyle, { color: palette.text }]}
        >
          Visit Collector
        </Animated.Text>
      </Pressable>
      <View
        {...elementProps(`sidebar-navigation`, scope)}
        style={styles.navigation}
      >
        {items.map(({ value, title, icon }) => button(title, icon, () => onNavigate(value), value, value === active))}
      </View>
      <View
        {...elementProps(`sidebar-bottom`, scope)}
        style={styles.bottom}
      >
